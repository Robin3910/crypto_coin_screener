...
if __name__=='__main__':
    ...
    for usdt in usdts:
        try:
            ...
            maclose1,maclose2,maclose3,maopen1,maopen2,maopen3 = crawl_exchanges_dates('binance', usdt, '15m', 2, 2, 2, 2, 2, 2, 2, 2, 15)
            ...
            if maclose1 > maopen1 and maclose2 > maopen2 and maclose3 > maopen3 and maopen2 > maclose1 and maopen3 > maclose2:
                ...
                print(f'{usdt}符合')
                print(binan4Hk)
                print(f'币安MA15分钟连续三根阳线，且实线部分最低点依次比上一根最高点高测试一共{len(binan4Hk)}个')
                print("*" * 40)
            else:
    ...
    file_dir = os.path.join(current_path,'币安MA15分钟连续三根阳线，且实线部分最低点依次比上一根最高点高测试')
    ...
    filename = str(caiji_time) + '点MA15分钟连续三根阳线，且实线部分最低点依次比上一根最高点高测试所有币' + '.txt'
    with open(f'{file_dir}//{filename}', 'w') as f:
        f.write(str(binan4Hk))

    filename1 = str(caiji_time) + '点MA15分钟连续三根阳线，且实线部分最低点依次比上一根最高点高MA价格' + '.txt'
