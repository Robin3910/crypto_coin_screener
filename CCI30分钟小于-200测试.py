...
if __name__=='__main__':
    ...
    for usdt in usdts:
        try:
            ...
            hma5, hma10, hma20, hma30, hma60, hma90, hma144, hma169, hmaclose, hmaopen, hmahigh, hmalow, k, d, j,cci = crawl_exchanges_dates('binance', usdt, '30m', 5, 6, 7, 8, 9, 10, 11, 15, 30)
            ...
            if cci < -200:
                ...
                print(binan4Hk)
                print(f'币安30分钟CCI小于-200一共{len(binan4Hk)}个')
                print("*" * 40)
            else:
    ...
    file_dir = os.path.join(current_path,'币安30分钟CCI小于-200测试')
    ...
    filename = str(caiji_time) + '点ma30分钟CCI小于-200测试所有币' + '.txt'
    with open(f'{file_dir}//{filename}', 'w') as f:
        f.write(str(binan4Hk))

    filename1 = str(caiji_time) + '点30分钟CCI小于-200收盘价格' + '.txt'
