...
if __name__=='__main__':
    ...
    for usdt in usdts:
        try:
            ...
            hma5, hma10, hma20, hma30, hma60, hma90, hma144, hma169, hmaclose, hmaopen, hmaclose1,hmaopen1,hmaclose2 = crawl_exchanges_dates('binance', usdt, '4h', 5, 10, 20, 30, 60, 90, 144, 169, 240)
            ...
            if hmaclose1 < hmaopen1 and hmaclose1 < hmaclose2 and hmaclose / hmaclose1 > 1.03 and hmaopen <= hmaclose1 and hmaclose > hmaopen1:
                ...
                print(f'{usdt}符合')
                print(binan4Hk)
                print(f'币安4小时吞没形态一共{len(binan4Hk)}个')
                print("*" * 40)
            else:
    ...
    file_dir = os.path.join(current_path,'币安4小时吞没形态测试')
    ...
    filename = str(caiji_time) + '点ma4小时吞没形态测试所有币' + '.txt'
    with open(f'{file_dir}//{filename}', 'w') as f:
        f.write(str(binan4Hk))

    filename1 = str(caiji_time) + '点ma4小时吞没形态MA价格' + '.txt'
