...
if __name__=='__main__':
    ...
    for usdt in usdts:
        try:
            ...
            if mma6010 > mmaclose > mma60:
                ...
                print(binan4Hk)
                print(f'币安MA日线价格在60上方百分之十以内一共{len(binan4Hk)}个')
                print("*" * 40)
    ...
    file_dir = os.path.join(current_path,'币安MA日线价格在60上方百分之十以内测试')
    ...
    filename = str(caiji_time) + '点MA日线价格在60上方百分之十以内测试所有币' + '.txt'
    with open(f'{file_dir}//{filename}', 'w') as f:
        f.write(str(binan4Hk))

    filename1 = str(caiji_time) + '点MAMA日线价格在60上方百分之十以内当前收盘价格' + '.txt'
