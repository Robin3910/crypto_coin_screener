...
if __name__=='__main__':
    ...
    for usdt in usdts:
        try:
            ...
            hma5, hma10, hma20, hma30, hma60, hma90, hma144, hma169, hmaclose, hmaopen, hmahigh, hmalow = crawl_exchanges_dates('binance', usdt, '1d', 5, 6, 7, 8, 9, 10, 11, 12, 1440)
            ...
            if -0.02 <= hmaclose - hmaopen < 0.02 and hmahigh - hmaclose > 0.03 and hmaclose - hmalow > 0.03:
                ...
                print(binan4Hk)
                print(f'币安1日十字星一共{len(binan4Hk)}个')
                print("*" * 40)
            else:
    ...
    file_dir = os.path.join(current_path,'币安1日十字星测试')
    ...
    filename = str(caiji_time) + '点ma1日十字星测试所有币' + '.txt'
    with open(f'{file_dir}//{filename}', 'w') as f:
        f.write(str(binan4Hk))

    filename1 = str(caiji_time) + '点1日十字星MA价格' + '.txt'
