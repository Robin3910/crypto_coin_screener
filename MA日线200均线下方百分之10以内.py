...
def crawl_exchanges_dates(exchange_name,symbol,start_time,end_time):
    try:    
        ...
        if  ma200 > ma > ma20010:
            ...
            print(binan4Hk)
            print(f'币安日线200均线下方百分之10以内一共{len(binan4Hk)}个')
            print("*" * 40)
    except:
...
if __name__=='__main__':
    ...
    current_path = os.getcwd()
    file_dir = os.path.join(current_path,'币安日线200均线下方百分之10以内')

    if not os.path.exists(file_dir):
        os.makedirs(file_dir)

    caiji_time = time.strftime("%Y-%m-%d日%H", end)
    filename = str(caiji_time) + '点ma在日线200均线下方百分之10以内所有币' + '.txt'
    with open(f'{file_dir}//{filename}', 'w') as f:
        f.write(str(binan4Hk))
    # print(f'{str(caiji_time)}点采集完毕')

    filename1 = str(caiji_time) + '点ma日线200均线下方百分之10以内价格' + '.txt'
