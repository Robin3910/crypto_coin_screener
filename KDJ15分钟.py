...
def crawl_exchanges_dates(exchange_name,symbol,timeframe,aver1,aver2,aver3,aver4,aver5,aver6,aver7,aver8,manymin):
    try:    
        ...
        d = df.iloc[-2]['slowd']
        j = df.iloc[-2]['slowj']
        return x1,x2,x3,x4,x5,x6,x7,x8,maclose,maopen,mahigh,malow,k,d,j
...
if __name__=='__main__':
    ...
    for usdt in usdts:
        try:
            ...
            hma5, hma10, hma20, hma30, hma60, hma90, hma144, hma169, hmaclose, hmaopen, hmahigh, hmalow, k, d, j = crawl_exchanges_dates('binance', usdt, '15m', 5, 6, 7, 8, 9, 10, 11, 12, 15)
            ...
            if k > d and k < 50:
                ...
                print(binan4Hk)
                print(f'币安15分钟KDJ一共{len(binan4Hk)}个')
                print("*" * 40)
            else:
    ...
    file_dir = os.path.join(current_path,'币安15分钟KDJ测试')
    ...
    filename = str(caiji_time) + '点ma15分钟KDJ测试所有币' + '.txt'
    with open(f'{file_dir}//{filename}', 'w') as f:
        f.write(str(binan4Hk))

    filename1 = str(caiji_time) + '点15分钟KDJ收盘价格' + '.txt'
